...
export default {
  ...
  data() {
    return {
      ...
      unTrashButtons: [
        {
          name: this.$t('api_test.automation.batch_updateCsvFile'),
          handleClick: this.handleBatchUpdateCsvFile,
  ...
  created() {
    ...
    methods: {
      ...
      handleBatchUpdateCsvFile() {
        if (this.trashEnable) {
          //let ids = Array.from(this.selectRows).map(row => row.id);
          let param = {};
          this.buildBatchParam(param);
          this.result.loading = true;
          this.$post('/api/automation/updateCsvFileBatchByCondition/', param, () => {
          ...
          let param = {};
          this.buildBatchParam(param);
          let alertMsg = this.$t('load_test.update_threadgroup_confirm') + " ？";
          this.$alert(alertMsg, '', {
            confirmButtonText: this.$t('commons.confirm'),
            cancelButtonText: this.$t('commons.cancel'),
            callback: (action) => {
              if (action === 'confirm') {
                this.$post('/api/automation/updateCsvFileBatchByCondition/', param, response => {
                  this.$success(this.$t(response.data));
                  this.search();
                });
              }
            }
